        ...
        private void Update()
        {
            ...
            if (m_StartedSound)
            {
                ...
                else
                {
                    ...
                    float decFade = .75f - accFade;

                    // get the high fade value based on the cars revs
                    float highFade = Mathf.InverseLerp(0.2f, 0.8f, m_CarController.Revs);
                    float lowFade = .75f - highFade;

                    // adjust the values to be more realistic
                    highFade = .75f - ((.75f - highFade)*(.75f - highFade));
                    lowFade = .75f - ((.75f - lowFade)*(.75f - lowFade));
                    accFade = .75f - ((.75f - accFade)*(.75f - accFade));
                    decFade = .75f - ((.75f - decFade)*(.75f - decFade));
